...
export const TrajectoryViewer: React.FC<TrajectoryViewerProps> = ({ repoName, steps }) => {
  ...
  return (
    <div className="w-full bg-[#F7F5EE] border-4 border-[#2A2E38] p-6 shadow-[8px_8px_0px_0px_#2A2E38] font-sans my-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b-4 border-[#2A2E38] pb-4 mb-6 gap-4">
        <div>
          <div className="flex items-center gap-2 text-xs font-mono font-bold text-[#5A5E6B] uppercase">
            <Cpu className="w-4 h-4 text-[#1E8E5A]" />
            AGENT TRAJECTORY & TOOL EXECUTION TRACE
          </div>
          <h3 className="text-2xl font-black text-[#15181F]" style={{ fontFamily: 'var(--font-bricolage), sans-serif' }}>
            Trajectory Log: {repoName}
          </h3>
        </div>
        <div className="text-xs font-mono bg-[#2A2E38] text-[#F7F5EE] px-3 py-1.5 font-bold">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Step Selector List */}
        <div className="space-y-2 max-h-[420px] overflow-y-auto border-2 border-[#2A2E38] p-2 bg-white">
          {steps.map((s, idx) => (
            <button
              key={idx}
              onClick={() => setSelectedStepIndex(idx)}
              className={`w-full text-left p-3 border-2 transition-all duration-150 flex items-center justify-between ${
        ...
        {/* Step Detail View */}
        <div className="md:col-span-2 border-2 border-[#2A2E38] bg-[#15181F] text-[#F7F5EE] p-5 shadow-[4px_4px_0px_0px_#2A2E38] font-mono text-xs overflow-x-auto">
